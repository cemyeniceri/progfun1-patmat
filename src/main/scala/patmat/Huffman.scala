...
object Huffman {
   ...
   */
    ...
    def timesRec(chars: List[Char], acc :List[(Char, Int)]): List[(Char, Int)] ={
      if(chars.isEmpty)
        acc
      else{
        val existentPair = acc.find(p => p._1.equals(chars.head))
        if(existentPair.isDefined)
          timesRec(chars.tail, acc.filterNot(p=> p == existentPair.get) :+ (existentPair.get._1, existentPair.get._2 +1) )
        else
          timesRec(chars.tail, acc :+ (chars.head,1))
      }
    }
    timesRec(chars, List())
  }
   ...
   */
    ...
    def loop(treesAcc: List[CodeTree]): CodeTree ={
      if(singleton(treesAcc))
        treesAcc.head
      else
        loop(combine(treesAcc))
    }
    loop(trees)
  }
  /**
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      val wholeTree = tree
      def loop(tree: CodeTree) :CodeTable ={
        tree match {
          case Fork(left, right, _, _) => mergeCodeTables(loop(left), loop(right))
          case Leaf(c, _) => List((c, encode(wholeTree)(List(c))))
        }
      }
      loop(tree)
    }
